...
def plan_trip (line_start, start, line_stop, stop)
  ...
  if line_start == line_stop
    ...
    p "You must travel through the following stops on the #{line_start} line: #{new_trip}"
    p "#{total_stop} stops in total."
    ...
    first_trip_length = first_trip.length
     # check union square to end stops
      ...
      #   second_trip = @train_line[line_stop][end_stop..union_line_stop]
      end
     #find first line lenght

     # find second line length

     # add togther - 1

     total_stop = first_trip.length + second_trip.length - 1
     p "You must travel through the following stops on the #{line_start} line: #{first_trip}"
     p "Chnage at Union Square"
     p "Your journey continues through the following stops: #{second_trip}"
     p "#{total_stop} stops in total."




    # train_line["N"].index "Union Square"
